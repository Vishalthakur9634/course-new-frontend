            ...
            <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
                ...
                <div className="flex-1 space-y-8 md:space-y-16">
                    ...
                    <motion.section
                        ...
                        className="bg-[#141414] border border-white/5 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 lg:p-16 relative overflow-hidden shadow-3xl group"
                    >
                            ...
                            <div className="flex-1 text-center lg:text-left space-y-6 md:space-y-10 w-full">
                                <div className="space-y-2 md:space-y-4">
                                    <div className="flex items-center justify-center lg:justify-start gap-2 md:gap-3">
                                        <div className="w-1.5 h-8 md:h-8 bg-brand-primary rounded-full shadow-[0_0_15px_rgba(255,161,22,0.5)]" />
                                        <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter uppercase leading-none">
                                            Hello, <span className="text-brand-primary">{user?.name?.split(' ')[0] || 'User'}</span>
                                        </h1>
                                    </div>
                                    <p className="text-[10px] md:text-[11px] text-dark-muted font-black uppercase tracking-[0.2em] md:tracking-[0.5em] opacity-40">Tactical Education Control Center</p>
                                </div>

                                <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-3 md:gap-6 w-full sm:w-auto mt-4 md:mt-0">
                                    <Link to="/my-learning" className="bg-brand-primary text-dark-bg px-6 md:px-12 py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase text-[11px] tracking-widest hover:brightness-110 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 shadow-2xl shadow-brand-primary/20 group/btn w-full sm:w-auto">
                                        <PlayCircle size={18} className="md:w-5 md:h-5 group-hover/btn:scale-125 transition-transform" /> Continue Mission
                                    </Link>
                                    <Link to="/browse" className="bg-white/5 text-white px-6 md:px-12 py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase text-[11px] tracking-widest hover:bg-white/10 hover:-translate-y-1 border border-white/10 transition-all flex items-center justify-center gap-3 shadow-xl w-full sm:w-auto">
                                        <Search size={18} className="md:w-5 md:h-5 text-brand-primary" /> Scan Catalog
                                    </Link>
                                </div>
                            </div>

                            {/* USER PERFORMANCE HUB */}
                            <div className="w-full lg:max-w-[400px] bg-black/40 backdrop-blur-2xl p-6 md:p-10 rounded-[1.5rem] md:rounded-[2rem] border border-white/10 shadow-3xl space-y-6 md:space-y-8 relative overflow-hidden">
                                <div className="flex justify-between items-end border-b border-white/5 pb-4 md:pb-6">
                                    <div className="space-y-1">
                                        ...
                                        <div className="text-2xl md:text-3xl font-black text-white italic tracking-tighter">LEVEL {Math.floor((user?.gamification?.xp || 0) / 1000) + 1}</div>
                                    </div>
                                    <div className="text-xs md:text-sm font-black text-brand-primary uppercase tracking-widest">{user?.gamification?.xp || 0} XP</div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 md:gap-6">
                                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl border border-white/5 group/stat hover:border-brand-primary/30 transition-all">
                                        <div className="flex items-center gap-2 md:gap-3 mb-2 text-dark-muted group-hover/stat:text-brand-primary transition-colors">
                                            <Target size={12} className="md:w-[14px] md:h-[14px]" />
                                            <p className="text-[9px] font-black uppercase tracking-widest">Completed</p>
                                        </div>
                                        <p className="text-2xl md:text-4xl font-black text-white tracking-tighter">{completedCourses}</p>
                                    </div>
                                    <div className="bg-white/5 p-4 md:p-6 rounded-2xl border border-white/5 group/stat hover:border-brand-primary/30 transition-all">
                                        <div className="flex items-center gap-2 md:gap-3 mb-2 text-dark-muted group-hover/stat:text-brand-primary transition-colors">
                                        ...
                                        <p className="text-2xl md:text-4xl font-black text-white tracking-tighter">{totalHours}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <section className="space-y-6 md:space-y-10">
                            <div className="flex justify-between items-end border-b border-white/5 pb-4 md:pb-8">
                                <div className="space-y-1 md:space-y-2">
                                    <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight flex items-center gap-3 md:gap-4">
                                        <TrendingUp className="text-brand-primary w-6 h-6 md:w-8 md:h-8" /> Active Coordination
                                    </h2>
                                    <p className="text-[10px] text-dark-muted font-black uppercase tracking-[0.3em] opacity-40">Resuming Critical Learning Nodes</p>
                                </div>
                                    ...
                                    >
                                        <Link to={`/course/${enrollment.courseId._id}`} className="bg-[#141414] border border-white/5 rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 hover:border-brand-primary/40 transition-all group block shadow-xl hover:shadow-brand-primary/5 relative overflow-hidden h-full flex flex-col">
                                            ...
                                            <div className="space-y-2 md:space-y-4 px-1 pb-1 flex-1">
                                                <div className="flex justify-between items-center text-[10px] font-black text-brand-primary uppercase tracking-[0.2em] mb-1">
                                                    <span>{enrollment.courseId.category}</span>
                                                    <span>{Math.round(enrollment.progress)}%</span>
                                                </div>
                                                <h3 className="font-black text-lg md:text-xl text-white line-clamp-2 uppercase tracking-tight group-hover:text-brand-primary transition-colors">{enrollment.courseId.title}</h3>
                                            </div>
                                        </Link>
                                    </motion.div>
                                ))}
                    ...
                    <section className="space-y-8 md:space-y-12">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-10 border-b border-white/5 pb-6 md:pb-10">
                            <div className="space-y-2 md:space-y-3">
                                <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-white uppercase tracking-tighter flex items-center gap-3 md:gap-5">
                                    <Rocket size={24} className="md:w-9 md:h-9 text-brand-primary" /> Exploration Hub
                                </h2>
                                <p className="text-dark-muted font-black text-[10px] md:text-[11px] uppercase tracking-[0.3em] md:tracking-[0.5em] opacity-40">Curated Intelligence Recommendation Matrix</p>
                            </div>
                            <div className="relative w-full md:max-w-[450px]">
                                <Search className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 text-dark-muted group-focus-within:text-brand-primary transition-colors w-5 h-5" size={20} />
                                <input
                                    type="text"
                                    placeholder="SCAN CURRICULUM ARCHIVE..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="w-full bg-[#111] border border-white/10 rounded-xl md:rounded-[1.5rem] py-4 md:py-5 pl-12 md:pl-16 pr-5 md:pr-8 text-[11px] md:text-xs font-black text-white placeholder:text-dark-muted uppercase tracking-widest focus:border-brand-primary/40 focus:ring-1 focus:ring-brand-primary/20 focus:outline-none transition-all shadow-3xl"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2.5 md:gap-4 overflow-x-auto pb-4 px-4 md:px-0 -mx-4 md:-mx-0 remove-scrollbar">
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`px-6 md:px-10 py-3 md:py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border whitespace-nowrap overflow-hidden relative group/pill flex-shrink-0 ${selectedCategory === cat
                            ...
                            <AnimatePresence mode="popLayout">
                                {filteredCourses.slice(0, 6).map((course, idx) => (
                                    <motion.div
                                        ...
                                        className="min-w-[85vw] md:min-w-0 snap-center h-auto"
                                    >
                                        <Link to={`/course/${course._id}`} className="group bg-[#141414] border border-white/5 rounded-[1.5rem] md:rounded-[2rem] overflow-hidden hover:border-brand-primary/30 transition-all duration-700 flex flex-col h-full shadow-2xl relative">
                                            <div className="aspect-[16/10] bg-black relative overflow-hidden ring-1 ring-white/5 flex-shrink-0">
                                                ...
                                                <div className="absolute top-4 right-4 md:top-6 md:right-6">
                                                    <span className="bg-black/60 backdrop-blur-xl text-white px-3 py-1.5 md:px-4 md:py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border border-white/10 shadow-3xl">
                                                        {course.category}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="p-5 md:p-8 lg:p-10 flex-1 flex flex-col space-y-4 md:space-y-8">
                                                <h3 className="text-xl md:text-xl lg:text-2xl font-black text-white group-hover:text-brand-primary transition-colors tracking-tight line-clamp-2 uppercase leading-snug">
                                                    {course.title}
                                                </h3>
                                                <div className="flex justify-between items-center pt-4 md:pt-8 border-t border-white/5 mt-auto">
                                                    <div className="flex items-center gap-2 md:gap-3 text-yellow-500/80 font-black text-[10px] uppercase tracking-widest">
                                                        <Star size={14} className="md:w-[16px] md:h-[16px]" fill="currentColor" />
                                                        <span>High Fidelity</span>
                                                    </div>
                                                    <div className="flex items-center gap-4">
                                                        <span className="font-black text-2xl md:text-3xl text-white tracking-tighter italic">${course.price}</span>
                                                        <button
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                e.stopPropagation();
                                                            ...
                                                            className="p-2.5 md:p-3 bg-brand-primary/10 hover:bg-brand-primary text-brand-primary hover:text-dark-bg rounded-xl border border-brand-primary/20 transition-all group/buy"
                                                        >
                                                            <ShoppingCart size={18} className="md:w-[18px] md:h-[18px]" />
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div className="w-full lg:max-w-[380px] xl:max-w-[400px] space-y-8 md:space-y-12 flex-shrink-0 mt-8 lg:mt-0">

                    {/* LIVE SIGNAL */}
                    <div className="bg-[#141414] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 shadow-3xl space-y-8 md:space-y-10 relative overflow-hidden group">
                        ...
                        <div className="space-y-6 md:space-y-10">
                            <div className="flex items-center justify-between border-b border-white/5 pb-5 md:pb-10">
                                <div className="space-y-2">
                                    <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter italic">Neural Pulse</h3>
                                    <p className="text-[10px] font-black text-brand-primary uppercase tracking-[0.4em] md:tracking-[0.5em] opacity-80 animate-pulse flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-brand-primary" /> Live Signal
                                    </p>
                                </div>
                                <div className="p-3 md:p-5 bg-brand-primary/10 rounded-xl md:rounded-[1.5rem] text-brand-primary border border-brand-primary/20 shadow-2xl shadow-brand-primary/10">
                                    ...
                                    {following.slice(0, 6).map((f, i) => (
                                        ...
                                        >
                                            <motion.div
                                                ...
                                                className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-[#0a0a0a] ring-1 ring-white/10 p-0.5 md:p-1 group-hover/avatar:ring-brand-primary/50 transition-all overflow-hidden shadow-2xl"
                                            >
                                            ...
                                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 md:w-4 md:h-4 bg-green-500 rounded-full border-2 md:border-[3px] border-[#141414] shadow-2xl" />
                                        </Link>
                                    ))}
                                    <button className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white/5 border-2 border-dashed border-white/10 flex items-center justify-center text-dark-muted hover:border-brand-primary hover:text-white transition-all shadow-xl group/add">
                                        <Plus size={16} className="md:w-[20px] md:h-[20px] group-hover/add:rotate-90 transition-transform" />
                                    </button>
                                </div>
                            </div>
                                ...
                                {socialPulse.length > 0 ? socialPulse.map((post, idx) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 md:gap-4">
                                            <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-brand-primary shadow-xl flex items-center justify-center text-dark-bg text-xs md:text-sm font-black italic">
                                                {post.author?.name?.charAt(0)}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs md:text-sm font-black text-white uppercase tracking-tight truncate">{post.author?.name}</span>
                                                ...
                                                <p className="text-[9px] text-brand-primary font-black uppercase tracking-widest opacity-60 mt-0.5">Transmission Active</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-6 md:gap-8 pt-1">
                                            <div className="flex items-center gap-2 md:gap-3 text-[10px] font-black text-brand-primary uppercase tracking-widest cursor-pointer hover:scale-105 transition-transform">
                                                <MessageSquare size={14} className="md:w-[16px] md:h-[16px]" /> <span>{post.comments?.length || 0}</span>
                                            </div>
                                            <div className="flex items-center gap-2 md:gap-3 text-[10px] font-black text-red-500 uppercase tracking-widest cursor-pointer hover:scale-105 transition-transform">
                    ...
                    <motion.div
                        ...
                        className="bg-brand-primary border border-brand-primary/20 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 relative overflow-hidden group text-center shadow-39 shadow-brand-primary/20 cursor-pointer"
                    >
                        ...
                        <div className="relative z-10 space-y-6 md:space-y-10">
                            <div className="w-16 h-16 md:w-24 md:h-24 rounded-xl md:rounded-[1.5rem] bg-dark-bg flex items-center justify-center text-brand-primary mx-auto shadow-3xl group-hover:rotate-[360deg] transition-transform duration-1000 border-2 border-brand-primary/20">
                                <Rocket size={24} className="md:w-[40px] md:h-[40px]" />
                            </div>
                            <div className="space-y-2 md:space-y-4">
                                <h4 className="text-2xl md:text-4xl font-black text-dark-bg uppercase tracking-tighter italic">AI MENTOR</h4>
                                <p className="text-[10px] md:text-sm text-dark-bg font-black leading-relaxed px-1 md:px-6 opacity-80 uppercase tracking-tight">
                                    Accelerate your domain mastery with high-fidelity predictive guidance.
                                </p>
                            </div>
                            <Link to="/neural-tutor" className="block w-full py-4 md:py-6 bg-dark-bg text-brand-primary rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-[12px] tracking-[0.3em] md:tracking-[0.4em] hover:scale-105 active:scale-95 transition-all shadow-2xl">
                                INITIATE AGENT
                            </Link>
                        </div>
                    </motion.div>
